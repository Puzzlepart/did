...
export class TimesheetService {
	private readonly _confirmed_periods: Collection;
	private readonly _forecasted_periods: Collection;
	private readonly _time_entries: Collection;
	private readonly _forecasted_time_entries: Collection;
	 ...
	 */
	public async getTimesheet(parameters: IGetTimesheetParams): Promise<any[]> {
		try {
			...
			for (let i = 0; i < periods.length; i++) {
				...
				periods[i].isForecasted = Boolean(forecasted);
	 ...
	 */
	public async submitPeriod(parameters: ISubmitPeriodParams): Promise<void> {
		try {
			const events = await this._msgraph.getEvents(
	...
	private _createUniqueEventId(eventId: string, startDateTime: Date) {
		return `${eventId}${startDateTime.getTime()}`.replace(/[^a-zA-Z\d]/g, '');
	}
	...
	private _getPeriodData(id: string, userId: string) {
		...
		return {
			_id: `${id}${userId}`.replace(/[^a-zA-Z\d]/g, ''),
			_userId: userId,
			week,
			month,
			year
