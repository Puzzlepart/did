...
export default class {
	constructor(private readonly _data: ProjectsData) {}
	...
	private _searchString(
		...
		let regex = /((?<customerKey>[\wæøå]{2,}?)\s(?<key>[\wæøå]{2,}))/gim;
		if (strictMode) {
			regex = /[({[]((?<customerKey>[\wæøå]{2,}?)\s(?<key>[\wæøå]{2,}?))[)\]}]/gim;
		}
	...
	private _matchEvent(event: EventObject) {
		...
		// We search the whole srchStr for match in non-strict/soft mode
			...
			event.project = find(this._data.projects, ({_id}) =>
				Boolean(find(softMatches, m => m.id === _id))
			);
			event.customer = find(
				this._data.customers,
	...
	private _checkInactive(event: EventObject) {
		...
		if (event.project && (inactiveProject || inactiveCustomer)) {
			if (inactiveProject) {
				event.error = {code: 'PROJECT_INACTIVE'};
			}

			if (inactiveCustomer) {
				event.error = {code: 'CUSTOMER_INACTIVE'};
			}
