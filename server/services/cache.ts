...
export class CacheService {
	...
	private _getScopedCacheKey(key: CacheKey, scope: CacheScope = this.scope) {
		key = isArray(key) ? filter(key, k => Boolean(k)) : [key];
		return [
			this.prefix,
			...key,
	 ...
	 */
	private async _get<T = any>({key, scope}: CacheOptions): Promise<T> {
	 ...
	 */
	private async _set<T = any>({key, scope, expiry}: CacheOptions, value: T) {
		return new Promise(resolve => {
			const scopedCacheKey = this._getScopedCacheKey(key, scope);
			log(
	 ...
	 * @param options - Cache options
	 */
	public async clear({key, scope}: CacheOptions) {
