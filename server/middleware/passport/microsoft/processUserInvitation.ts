...
export async function processUserInvitation(
  ...
  debug(`Registering user ${profile.preferred_username} (${profile.oid}) with subscription ${userInvitation.subscription.id}`)
  await subSvc.registerExternalUser(PROVIDER, profile.oid, userInvitation.subscription.id)
