...
import { FetchPolicy, QueryLazyOptions, useLazyQuery, useQuery } from '@apollo/client'
...
export function useReportsQuery(
  { state, dispatch, fetchPolicy = 'no-cache' }: useReportsQuery 
): (options?: QueryLazyOptions<any>) => void {
  const [query, queryResult] = useLazyQuery(state.queryPreset?.query || default_query, {
    fetchPolicy,
