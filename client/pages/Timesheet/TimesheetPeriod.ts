...
export class TimesheetPeriod {
	...
	private readonly events: EventObject[] = [];
	...
	public getEvents(includeUnmatched: boolean = true): EventObject[] {
		return [...(this.events || [])]
			.filter(event => {
				...
				const isMatched = Boolean(event.project);
				return (
	...
	public get errors(): any[] {
		...
		return filter(this.getEvents(), event => Boolean(event.error)).map(
			event => event.error
		);
	}
	...
	public get matchedDuration(): number {
		return filter(this.getEvents(), event => Boolean(event.project)).reduce(
	...
	private get matchedEvents(): EventInput[] {
		const events = filter([...this.getEvents()], event =>
			Boolean(event.project)
		).map(
			event =>
				({
					id: event.id,
