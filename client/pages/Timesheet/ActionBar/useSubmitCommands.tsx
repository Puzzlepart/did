...
export function useSubmitCommands() {
	...
	return {
		key: 'SUBMIT_COMMANDS',
		onRender: () => {
			if (Boolean(context.error) || !context.selectedPeriod) {
				return null;
			}
			...
			const cmd: Record<string, IContextualMenuItem> = {
			...
			if (isConfirmed) {
				commands.push(cmd.UNCONFIRM_PERIOD);
			} else if (isForecast) {
				if (isComplete) {
				...
				if (!isPast) {
					if (isForecasted) {
						commands.push(cmd.UNFORECAST_PERIOD);
					} else {
						commands.push(cmd.FORECAST_PERIOD);
					}
				}
			} else {
				if (!isPast) {
