...
test('BrowserStorage: stores and retrieves arrays', (t) => {
  const storage = new BrowserStorage<number[]>('numbers', t.context.mockStore as any)
...
test('BrowserStorage: prefixes key with "did_"', (t) => {
  const mockStore = t.context.mockStore as MockStorage
...
test('BrowserStorage: returns fallback when key does not exist', (t) => {
  const storage = new BrowserStorage<string>('nonexistent', t.context.mockStore as any)
...
test('BrowserStorage: handles null value', (t) => {
  const storage = new BrowserStorage<any>('null-test', t.context.mockStore as any)
  storage.set(null)
...
test('BrowserStorage: handles undefined value', (t) => {
  const storage = new BrowserStorage<any>('undefined-test', t.context.mockStore as any)
...
test('BrowserStorage: handles empty string key', (t) => {
  const storage = new BrowserStorage<string>('', t.context.mockStore as any)
  storage.set('value')
  const result = storage.get()
  t.is(result, 'value')
})
...
test('BrowserStorage: handles very long keys', (t) => {
  ...
  const storage = new BrowserStorage<string>(longKey, t.context.mockStore as any)
  storage.set('value')
  const result = storage.get()
  t.is(result, 'value')
})
...
test('BrowserStorage: stores and retrieves deeply nested objects', (t) => {
  const storage = new BrowserStorage<any>('nested', t.context.mockStore as any)
  const nested = {
    level1: {
      level2: {
        level3: {
...
test('BrowserStorage: handles empty object', (t) => {
  const storage = new BrowserStorage<object>('empty-obj', t.context.mockStore as any)
  storage.set({})
  const result = storage.get()
  t.deepEqual(result, {})
})
...
test('BrowserStorage: handles empty array', (t) => {
  const storage = new BrowserStorage<any[]>('empty-arr', t.context.mockStore as any)
  storage.set([])
  const result = storage.get()
  t.deepEqual(result, [])
})
...
test('BrowserStorage: stores and retrieves boolean true', (t) => {
  const storage = new BrowserStorage<boolean>('bool-true', t.context.mockStore as any)
  storage.set(true)
  const result = storage.get()
  t.is(result, true)
})
...
test('BrowserStorage: stores and retrieves boolean false', (t) => {
  const storage = new BrowserStorage<boolean>('bool-false', t.context.mockStore as any)
  storage.set(false)
  const result = storage.get()
  t.is(result, false)
})
...
test('BrowserStorage: stores and retrieves zero', (t) => {
  const storage = new BrowserStorage<number>('zero', t.context.mockStore as any)
  storage.set(0)
  const result = storage.get()
  t.is(result, 0)
})
...
test('BrowserStorage: stores and retrieves negative numbers', (t) => {
  const storage = new BrowserStorage<number>('negative', t.context.mockStore as any)
  storage.set(-42)
  const result = storage.get()
  t.is(result, -42)
})
...
test('BrowserStorage: stores and retrieves floating point numbers', (t) => {
  const storage = new BrowserStorage<number>('float', t.context.mockStore as any)
  storage.set(3.14159)
  const result = storage.get()
  t.is(result, 3.14159)
})
...
test('BrowserStorage: handles very large numbers', (t) => {
  const storage = new BrowserStorage<number>('large', t.context.mockStore as any)
...
test('BrowserStorage: setting same value multiple times is idempotent', (t) => {
  const storage = new BrowserStorage<string>('idempotent', t.context.mockStore as any)
  storage.set('value')
  storage.set('value')
  storage.set('value')
  const result = storage.get()
...
test('BrowserStorage: overwrites previous value', (t) => {
  const storage = new BrowserStorage<string>('overwrite', t.context.mockStore as any)
  storage.set('first')
  storage.set('second')
  const result = storage.get()
  t.is(result, 'second')
...
test('BrowserStorage: type mismatch - stores string when expecting object', (t) => {
  const storage = new BrowserStorage<{ name: string }>(
    'type-mismatch',
    t.context.mockStore as any
  )
  // Force type mismatch by using any
  storage.set('not-an-object' as any)
  const result = storage.get({ name: 'fallback' })
  // Should retrieve the string, not the fallback
  t.is(result, 'not-an-object')
})
...
test('BrowserStorage: different instances with different keys are isolated', (t) => {
  const storage1 = new BrowserStorage<string>('key1', t.context.mockStore as any)
  const storage2 = new BrowserStorage<string>('key2', t.context.mockStore as any)
...
test('BrowserStorage: stores and retrieves empty string', (t) => {
  const storage = new BrowserStorage<string>('empty-string', t.context.mockStore as any)
  storage.set('')
  const result = storage.get()
  t.is(result, '')
})
