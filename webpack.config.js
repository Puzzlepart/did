...
const HTML_PLUGIN_FILE_NAME = IS_DEVELOPMENT ?
	resolve(__dirname, 'server/views/@template_dev.hbs') :
	resolve(__dirname, 'server/views/@template.hbs');
	...
	plugins: [
		...
		new DefinePlugin({
			...
			'process.env.LOG_LEVEL': JSON.stringify(
				process.env.CLIENT_LOG_LEVEL || 'SILENT'
			)
		})
	]
};
...
if (IS_DEVELOPMENT) {
	...
	config.plugins.push(
		...
		new WebpackBuildNotifierPlugin({
			logo: resolve(
				__dirname,
				'/server/public/images/favicon/mstile-150x150.png'
			),
